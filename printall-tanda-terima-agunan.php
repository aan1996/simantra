...
class MYPDF extends TCPDF
{
  ...
  public function Header()
  {
    ...
    // Title
    $this->SetX(50);
    $this->Cell(0, 15, 'Bank Mandiri Kcp Morowali', 0, false, 'L', 0, '', 0, false, 'M', 'M');
    $this->ln(1);
    $this->SetX(50);
...
$pdf->SetTitle('Tanda Terima Agunan');
$pdf->SetSubject('Tanda Terima Agunan');
$pdf->SetKeywords('Tanda Terima Agunan');
...
$pdf->Cell(0, 0.7, 'Laporan Data Tanda Terima Agunan', 0, 0, 'C');
$pdf->ln(10);
$pdf->SetFont('dejavusans', '', 10, '', true);
...
<thead>
      <tr style="text-align: center;">
          ...
          <th colspan="2">Scan Surat</th>
          <th colspan="2">Keterangan Surat</th>
      </tr>
  </thead>
  <tbody>';
...
$result = mysqli_query($con, "SELECT * FROM tandaterimaagunan");
$no = 0;
while ($row = mysqli_fetch_array($result)) {
  ...
  $alamat = $row['alamat'];
  $images = $row['images'];
  $keterangansurat = $row['keterangansurat'];
  $no++;
  ...
  $tbl .= '<tr style="text-align: left;">
              ...
              <td colspan="2"><img src="upload/' . $row['images'] . '" width="150" height="75"></td>
              <td colspan="2">' . $row['keterangansurat'] . '</td>
          </tr>';
}
...
$pdf->ln(4);
$pdf->Cell(267, 0, "Branch Manager KCP Morowali Bahodopi", 0, 0, 'R');
$pdf->ln(20);
...
// This method has several options, check the source code documentation for more information.
ob_end_clean();
$pdf->Output('tanda-terima-agunan.pdf', 'I');
